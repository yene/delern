...
export const triggers = {
    deckShared: functions.database
        ...
        .onCreate(async (data, context) => {
            ...
            const deckName = (await admin.database()
                .ref('decks').child(sharedWithUser.uid).child(deckKey)
                .child('name').once('value')).val();
            const mailOptions = {
                ...
                subject: actorUser.displayName +
                    ' shared a Delern deck with you',
            ...
            for (const fcmId in fcmEntries) {
                ...
                payload.token = fcmId;
                try {
                    console.log('Notified:', await admin.messaging()
                        .send(payload));
                } catch (e) {
    ...
    cardAdded: functions.database.ref('/cards/{deckKey}/{cardKey}')
